...
module GeozonesHelper
  ...
  def prepare_geo_limitation_tag(taggable, resource_name, tag_filter_class)
    ...
    when 'only_geozones'
      geo_tags = ''
      taggable.geozone_limitations.each do |geozone|
        active_class = (@restricted_geozones.any? && @restricted_geozones.include?(geozone.id.to_s)) ? 'filtered-projekt' : ''
